 ...
 * @props sizeNum - 公有云后台size分页参数
 * @props sizeName - 公有云后台size参数名称
 * @props cloudBack - 公有云后台
  ...
  methods: {
    sortFn(idx){
      ...
      if(this.tagSort != idx){
        ...
        let _tgs = this.tagCls;
        this.tagSort = idx;
        this.tagCls = _tgs==2? 0 : ++_tgs;
        this.$emit('sortdispatch',idx,this.tagCls);
    },
    /**
     * 添加数据到组件
     *
    ...
    fetch(cb) {
      ...
      let cloudSize = {}
      cloudSize[this.sizeName] = this.sizeNum;

      ajaxHttp(this.url, {
        type: this.method,
        data:  this.cloudBack ? Object.assign(this.queryOpt, cloudSize) : this.queryOpt 
      }).then((rtn) => {
  ...
  events: {
    switchPage(currentPage) {
      ...
      if (this.url) {
        ...
        let curOpt = this.cloudBack ? { current: currentPage } : { _index: currentPage };

        this.queryOpt = Object.assign(this.queryOpt, curOpt);
        this.fetch();
