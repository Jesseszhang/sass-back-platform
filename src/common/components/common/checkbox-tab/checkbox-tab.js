	...
	methods: {
      ...
      this.datas.list = arr;
		...
		moveRight() {
			...
			let rightList = this.rightCheckData.list;
			let rightListValue = [];

			//存放右侧已存在的值
			rightList.map((item)=>{
				rightListValue.push(item.value);
			});
			...
			if (rightList.length === 0) {
				...
				arr.forEach((item)=>{
					//如果左侧选中的值在右侧未出现就增加，否则不做改变
					if (!~rightListValue.indexOf(item.value)) {
						this.rightCheckData.list.push(item);
					}
				});
			}
