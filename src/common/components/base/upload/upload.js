 ...
 * @props nameKey - 返回的文件名的 key 值
 * @props urlKey - 返回的文件地址的 key 值
 *
...
const Upload = {
  ...
  methods: {
    _init() {
      this.maxWidth = this.size.split('*')[0]
      this.maxHeight = this.size.split('*')[1]
      ...
      if(this.maxWidth > this.maxHeight){
        ...
        this.showImgStyle = {
          height: `${100 * _l * this.maxHeight / this.maxWidth}px`,
          width: 100 * _l +'px'
        }
      this._initRegex()
    },
     ...
     */
    scale(src) {
      this.scaleImgDisplay = true
      this.scaleImgSrc = src
    },
    ...
    checkPhotoSize(url, cb) {
      ...
      const imgComplete = () => {
        ...
        if (img.offsetHeight > this.maxHeight || img.offsetWidth > this.maxWidth) {
      ...
      $.ajax({
        ...
        success: (rtn) => {
          ...
          data.forEach((item, index) => {
            ...
            this.checkPhotoSize(item[this.urlKey], () => {
              this.uploadItems.push({
                url: item[this.urlKey],
                id: item[this.nameKey],
                width: this.width,
                height: this.height
              });
            })
