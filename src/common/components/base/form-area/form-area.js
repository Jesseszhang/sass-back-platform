 ...
 * @props isSort - 验证格式按指定的顺序验证
 --需要配合子组件添加errIdx(number)来指定顺序
 * @props valueAsUrl - 将 Url 值作为上传参数值
 * 
 * @slot - 加进来的表单控件
 *
 */
  ...
  methods: {
    ...
    _initFormData() {
      ...
      if(_self.isSort){
        ...
        let _arr = _self.$children;
         _arr.forEach((item,idx) => {
          item['errIdx'] = typeof item['errIdx']=='undefined'? 9999 :item['errIdx'];
        })
        _arr.sort(function(a,b){
          return a['errIdx']-b['errIdx']
        })
        this.$children = _arr;
      }
        ...
        comp.$children.forEach((comp, index) => {
          ...
          INIT_FORM_CONTROL.forEach((controlName) => {
            if (comp.constructor.name === controlName) {
              if (!comp.queryName || (typeof comp.val() === 'undefined')) {
                return false;
              }
    ...
    query (comp = {}, {toArray, empty} = {}) {
      ...
      var setQueryOpt = (comp, change, couple) => {
        if (change) {
          ...
          queryOpt[compQueryName].push(comp.val());
          ...
          if (couple) {
            ...
            queryInfo[compQueryName].push(comp.val());
          }
        }
      }
      ...
      switch (comp.constructor.name) {
        case 'DropMenu':
          ...
          queryOpt[compQueryName] = comp.val()
        ...
        default:
          ...
          queryOpt[compQueryName] = comp.val()
          queryInfo[compQueryName] = comp.val()
