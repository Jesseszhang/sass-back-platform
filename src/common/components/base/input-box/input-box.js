 ...
 * @props regex - 验证值的正则
 * @props type - 输入框类型( text | textarea | password )
 ...
 * @props errIdx -验证优先级
 * @props autoHeight -textarea是否根据内容自动伸缩
...
const InputBox = {
  ...
  ready(){
        if(this.type == TYPE_TEXT_AREA && this.value!=""){
          this.autoSizeInput();
        }
  },
  methods: {
    ...
    */
    autoSizeInput(){
      if(this.autoHeight == true && this.type == TYPE_TEXT_AREA){
        let _tgIpt = this.$el.getElementsByTagName('textarea')[0];
        let _ht = Math.max(_tgIpt.scrollHeight,96);
        this.styleObject = {
          height: _ht+'px'
        }
        //while(_et.target.scrollTop)
        /*var rowL = val.split("\n");
        this.row = Math.max(rowL.length+3,4);*/
      }
    },
